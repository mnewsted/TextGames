...
# TextGames
# version 1.5.9
# description: game 4 added cage trap crayfish
    ...
    if game_choice == '4':
        if event_id in [6, 7, 8]:
            ...
            if all(shell_list):
                ...
                world[17][
                    'desc'] = 'Tall reeds are thick in the northern edge of the lake. As you swim through the water, you see the surface above. A massive clam shell has opened revealing a passage to the north. The shallows extend to the east and to the west. Another cave entrance lies below.'
...
def do_event(event_id, current_room):
    ...
    if game_choice == '1':
        ...
        if event_id in [9]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
            player_hp = player_hp + random.randrange(1, 20) + 50
            if player_hp > 100:
                player_hp = 100
        ...
        if event_id in [10]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
            player_hp = player_hp + random.randrange(1, 20) + 55
            if player_hp > 100:
                player_hp = 100
        ...
        if event_id in [11]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
            player_hp = player_hp + random.randrange(1, 20) + 60
            if player_hp > 100:
                player_hp = 100
        ...
        if event_id in [15]:
            ...
            if things[item_index]['location'] == current_room or things[item_index]['on_person'] == True:
                print(events[event_id]['first_time_text'])
                events[event_id]['done'] = True
                # create new object - LOCATION CAN BE ANY IF ON_PERSON IS TRUE OTHERWISE MATCH LOCATION TO EVENT ROOM
                things.append({'name': 'rusty key', 'prefix': 'the',
        ...
        if event_id in [16]:
            ...
            events[event_id]['done'] = True
            # create new object - LOCATION CAN BE ANY IF ON_PERSON IS TRUE OTHERWISE MATCH LOCATION TO EVENT ROOM
            things.append({'name': 'broken trap', 'prefix': 'a',
            ...
            print('Crayfish is here. It is readies to attack!')
...
def load_game(choice):
    ...
    if choice == '1':
        ...
        things.append({'name': 'water grubs', 'prefix': 'some',
                       'description': 'If they weren\'t slimy, hairy, and wriggling, these beetle larvae might be cute. Nevermind that--you\'re more interested in the tasty protein they provide.',
        ...
        events.insert(16, {'id': 16, 'done': False, 'room': 40, 'item_name': 'lobster trap',
