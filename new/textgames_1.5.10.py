...
# TextGames
# version 1.5.10
# description: game 4 added outdoor events, fixed random numbers, made safe_rooms
        ...
        for mob in creatures:
            if mob['room'] == current_room and mob['is_dead'] == False:
                for thing in things:
                    ...
                    if (random.randrange(1, 101) + thing['hit_bonus']) > 50:
                        damage = thing['base_damage'] + random.randrange(1, (thing['damage'] + 1))
                        if weapon[-1] == 's':
...
def do_event(event_id, current_room):
    ...
    if game_choice == '1':
        ...
        if event_id in [9]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
            player_hp = player_hp + random.randrange(1, 21) + 40
            if player_hp > 100:
                player_hp = 100
        ...
        if event_id in [10]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
            player_hp = player_hp + random.randrange(1, 21) + 45
            if player_hp > 100:
                player_hp = 100
        ...
        if event_id in [11]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
            player_hp = player_hp + random.randrange(1, 21) + 50
            if player_hp > 100:
                player_hp = 100
        ...
        if event_id in [19]:
            ...
            print('You are now a MEGA-TURTLE.')
            player_hp = 100
            ...
            creatures.append({'id': 2, 'name': 'Sea serpent',
                              ...
                              'damage': 5, 'hit_bonus': 10, 'attack_chance': 70, 'is_fatigued': False, 'death_event': 20, 'was_seen': False,
            ...
            if current_room == '45':
                ...
                print('\nYou hear an unsettling sound in the distance. Something is slithering in the depths.')
        if event_id in [20]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
    ...
    if game_choice == '3':
        ...
        if (current_room in [30, 31, 32, 33]) and locale_visited == False:
            ...
            if special_chance >= 67: # harm
                ...
                if special_type == 1:
                    print('A hawk swoops down from the sky, delivering a mean scratch with its claws! It flies off before you can react.')
                elif special_type == 2:
                ...
                special_damage = random.randrange(1, 10)
                ...
                special_heal = random.randrange(1, 10)
...
def load_game(choice):
    ...
    if choice == '1':
        ...
        world.insert(34, {'visible': True, 'name': 'Antechamber', 'prefix': 'in the', 'name2': '',
                          'desc': 'Glowing creepy-crawlies provide the only light in this unfriendly pit. The way back to the well is above. You feel a stillness coming from the room to the south.',
                          'exits': {'u': 29, 's': 35}})
        ...
        events.insert(20, {'id': 20, 'done': False, 'room': 999, 'item_name': 'none',
                           'first_time_text': 'What an epic battle. That\'s it. You won.',
                           'already_done_text': 'The end.'})
...
#                          'first_time_text': 'You swing the hammer.', 'already_done_text': 'You swing the hammer.'})
        ...
        # sets the special event to apply to rooms 30-33
