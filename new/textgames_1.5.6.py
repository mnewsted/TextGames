...
# TextGames
# version 1.5.6
# description: game 4 fixed(?) attack with claws
        ...
        for mob in creatures:
            if mob['room'] == current_room and mob['is_dead'] == False:
                for thing in things:
                    ...
                    print(weapon)
                    print('hit_bonus: ' + str(thing['hit_bonus']) + ' damage: '+ str(thing['damage']))
...
def do_event(event_id, current_room):
    ...
    if game_choice == '1':
        ...
        if event_id in [5]:
            print(events[event_id]['first_time_text'])
            # item_index = ['claws' in i['name'] for i in things].index(True)
            # print('damage = ' + str(things[item_index]['damage']))
            events[event_id]['done'] = True
        if event_id in [0]:
            print(events[event_id]['first_time_text'])
            events[event_id]['done'] = True
        ...
        if event_id in [1]:
            item_index = ['note' in i['name'] for i in things].index(True)
            if things[item_index]['on_person'] == True:
                print(events[event_id]['first_time_text'])
                events[event_id]['done'] = True
                # create new object - LOCATION CAN BE ANY IF ON_PERSON IS TRUE OTHERWISE MATCH LOCATION TO EVENT ROOM
                things.append({'name': 'revealed note', 'prefix': 'the',
                               'description': 'After rinsing the note in the filtered water, additional words are visible! It reads:\n\nTo save the lake, go to the water\'s edge and use the ring of power.\nThe ring can be forged by one who is filled with fighting spirit, marked by symbols of great warriors.\nThe one who bears the ring may enter the lake to redeem it.',
                               'location': 3, 'on_person': True, 'moveable': True, 'is_weapon': False, 'no_drop': False})
                inventory_quantity = inventory_quantity + 1  ## add to inventory if appending thing on_person == True
                delete_thing('note')
            else:
                print('Nothing happened. Maybe try somewhere else? Or try when another item is near?')
        ...
        if event_id in [3]:
            necessary_parts = 0
            for thing in things:
                if thing['name'] == 'washer' and ( thing['location'] == current_room or thing['on_person'] == True ):
                    necessary_parts += 1
                if thing['name'] == 'shirt' and ( thing['location'] == current_room or thing['on_person'] == True ):
                    necessary_parts += 1
        ...
        if event_id in [4]:
            ...
            things.append({'name': 'trident', 'prefix': 'a',
                           'description': 'This three-pronged spear looks like it could puncture anything. Unfortunately, your short turtle-arms can\'t use it as a weapon. Still, you might find use for it.',
                           'location': current_room, 'on_person': False, 'moveable': True, 'is_weapon': False, 'no_drop': False})
...
def do_special(current_room):
    ...
    if game_choice == '3':
        ...
        if current_room == 12 and special_done[0]['done'] == False:
            ...
            item_index = ['claws' in i['name'] for i in things].index(True)
            things[item_index]['on_person'] = True
            inventory_quantity = inventory_quantity + 1  ## add to inventory if appending thing on_person == True
            special_done[0]['done'] = True
...
def load_game(choice):
    ...
    if choice == '1':
        ...
        exit_room = 2
        player_hp = 100
        intro_text = '*** Welcome to Lake Tortuga ***\n\nYou just got home from school. It was a long day. You aren\'t tired, but you feel a little uneasy. Maybe it\'s the note that someone stuffed in your pocket while you were riding the bus.\n\nThe note falls onto the floor as you try to grab it.\n'
        outro_text = 'Well done.'

        world.insert(0, {'visible': True, 'name': 'Foyer', 'prefix': 'in the', 'name2': '',
                         'desc': 'This entryway is just inside the front door of your home. Most of the house is accessible from here. The dining room lies to the west, the living room is south, a stairway leads to the second-level rooms, '
                                 'and a door that leads to the garage is east. On the northern wall is the front door.',
                         'exits': {'n': 9, 's': 4, 'w': 1, 'e': 5, 'u':6}})
        ...
        world.insert(7, {'visible': True, 'name': 'Bedroom', 'prefix': 'in your', 'name2': '',
                         'desc': 'A small but functional desk, a dresser, and a unmade bed take up space in the dim room. The door to the landing is east.',
                         'exits': {'e': 6}})
        world.insert(8, {'visible': True, 'name': 'Parent\'s bedroom', 'prefix': 'in your', 'name2': '',
                         'desc': 'A large bed dominates the space, which is emphasized by the wide mirror above the low dresser. The only exit is to the north.',
                         'exits': {'n': 6}})
        world.insert(9, {'visible': True, 'name': 'Street', 'prefix': 'on the', 'name2': '',
                         'desc': 'A few trees line this quiet single-lane road that winds past your house. Your house is directly south. From the north you can hear the water lap at the shore of the lake.',
                         'exits': {'s': 0, 'n': 10}})
        ...
        world.insert(17, {'visible': True, 'name': 'North shallows', 'prefix': 'in the', 'name2': '',
                          'desc': 'Tall reeds are thick in the northern edge of the lake. As you swim through the water, you see the surface above. The shallows extend to the east and to the west. Another cave entrance lies below.',
                          'exits': {'n': 36, 'e': 16, 'w': 18, 'd': 25}})
        world.insert(18, {'visible': True, 'name': 'Northwest shallows', 'prefix': 'in the', 'name2': '',
                          'desc': 'The water in the northwest corner of the lake is choked with silt. Clearings are visible to to the south and east. A massive branch from a long-dead tree provides a path up to the water\'s surface above.',
                          'exits': {'s': 19, 'e': 17, 'u': 32}})
        ...
        world.insert(22, {'visible': True, 'name': 'Southeast tunnel', 'prefix': 'in the', 'name2': '',
                          'desc': 'Jagged rocks make up the walls of this tunnel that runs from the southwest to the northeast.',
                          'exits': {'sw': 21, 'ne': 23}})
        ...
        world.insert(24, {'visible': True, 'name': 'Northeast tunnel', 'prefix': 'in the', 'name2': '',
                          'desc': 'This smooth, wide tunnel winds from the southeast to the northwest.',
                          'exits': {'e': 37, 'se': 23, 'nw': 25}})
        ...
        world.insert(26, {'visible': True, 'name': 'Northwest tunnel', 'prefix': 'in the', 'name2': '',
                          'desc': 'This plain passageway opens to the northeast and southwest.',
                          'exits': {'ne': 25, 'sw': 27}})
        ...
        world.insert(28, {'visible': True, 'name': 'Southwest tunnel', 'prefix': 'in the', 'name2': '',
                          'desc': 'This cramped passage runs in a northwest-to-southeast direction.',
                          'exits': {'nw': 27, 'se': 21}})
        ...
        world.insert(32, {'visible': True, 'name': 'Dead branch', 'prefix': 'on a', 'name2': '',
                          ...
                          'exits': {'d': 18}})
        world.insert(33, {'visible': True, 'name': 'Stoney mini-mesa', 'prefix': 'on a', 'name2': '',
        ...
        world.insert(34, {'visible': True, 'name': 'Antechamber', 'prefix': 'in the', 'name2': '',
                          'desc': 'Glowing creepy-crawlies provide the only light in this unfriendly pit. The doorway back to the well is above. You feel a stillness coming from the room to the south.',
                          'exits': {'u': 29, 's': 35}})
        world.insert(35, {'visible': True, 'name': 'Slimy core', 'prefix': 'in the', 'name2': '',
                          ...
                          'exits': {'n': 34}})
        world.insert(36, {'visible': True, 'name': 'Mermaid\'s grotto', 'prefix': 'in the', 'name2': '',
                          'desc': 'This area is a small but deep recess. Seashells cover the floor. There is a low door to the south.',
                          'exits': {'s': 17}})
        ...
        world.insert(38, {'visible': True, 'name': 'Tank room', 'prefix': 'in the', 'name2': '',
                          'desc': 'It seems like you are inside a voluminous, decay'
                                  'ing tank. Mechanical filters are choked with dark green mossy fibers. Wispy tendrils undulate from the ceiling and walls. The pipe that brought you into this place is to the east.',
                          'exits': {'w': 37}})
        world.insert(39, {'visible': True, 'name': 'Rocky passage', 'prefix': 'in a', 'name2': '',
                          'desc': 'This rough-hewn throughway appears to be carved by unnatural forces. It is a T-intersection, with exits to the north, northeast, and southwest.',
        ...
        world.insert(41, {'visible': True, 'name': 'Twisty tunnel', 'prefix': 'in a', 'name2': '',
                          'desc': 'Something must have carved throught the rock to make this jagged path. The ways out are to the west, northeast, and southwest.',
                          'exits': {'w': 40, 'ne': 28, 'sw': 39}})

        # hammer weapon is put in exit room - seems to fix claws attack bug
        things.append({'name': 'hammer', 'prefix': 'a',
                       'description': 'It\'s a classic ball-peen hammer. Could do some real damage.', 'location': 2,
                       'on_person': False, 'moveable': True, 'is_weapon': True, 'base_damage': 10, 'damage': 30, 'hit_bonus': 20, 'no_drop': False})
        things.append({'name': 'note', 'prefix': 'a',
                       'description': 'After you smooth out the crumpled note, you see writing on the top half of the page. It reads:\n\nTo save the lake, go to the water\'s edge and use the ring of power.\nThe ring can be forged by one who is...', 'location': 0,
                       'on_person': False, 'moveable': True, 'is_weapon': False, 'base_damage': 0, 'damage': 0, 'hit_bonus': 0, 'no_drop': False})
        things.append({'name': 'washer', 'prefix': 'a', 'description': 'This circular piece of steel looks sturdy.',
        ...
        events.insert(1, {'id': 1, 'done': False, 'room': 3, 'item_name': 'enhanced faucet',
                          'first_time_text': 'A stream of clear light pours from the faucet. You are compelled to put the note under the gentle flow, and when you do, letters begin to take shape on the blank part of the page!',
        ...
        events.insert(3, {'id': 3, 'done': False, 'room': 11, 'item_name': 'washer',
                           'first_time_text': 'Standing at the water\'s edge, wearing the Teenage Mutant Ninja Turtles shirt, the washer transforms into a ring. This metallic band fits snugly on your right index finger. '
                                              'You\'ve made the ring of power! You cannot remove the ring, but you can now descend into the lake!', 'already_done_text': 'Nothing happens.'})
        events.insert(4, {'id': 4, 'done': False, 'room': 999, 'item_name': 'trident',
...
#                          'first_time_text': 'You swing the hammer.', 'already_done_text': 'You swing the hammer.'})
        ...
        creatures.append({'id': 0, 'name': 'Mermaid',
                          ...
                          'status_neutral': 'She isn\'t interested in you, but eyes you warily.', 'status_hostile': 'She\'s very mad at you.',
                          'damage': 10, 'hit_bonus': 10, 'attack_chance': 75, 'is_fatigued': False, 'death_event': 4, 'was_seen': False,
                          'dead_description': 'The mermaid\'s lifeless body is elegant even in death.' })

        # sets the special event to apply to rooms 12-41
        ...
        # initialize special vars
        special_done.append({'id': 0, 'done': False}) # transformation message + claws
