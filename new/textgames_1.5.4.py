...
# TextGames
# version 1.5.4
# description: game 4 development more rooms, items, events
...
def do_special(current_room):
    ...
    if game_choice == '3':
        ...
        if current_room == 11:
            special_done[0]['done'] = False
            if special_done[1]['done'] == True:
                delete_thing('claws')
                print('You are a person again.')
                special_done[1]['done'] = False

        if current_room >= 12:
            print('You are a turtle.')
            if current_room == 12:
                if special_done[0]['done'] == False:
                    ...
                    special_done[0]['done'] = True
                    if special_done[1]['done'] == False:
                        things.append({'name': 'claws', 'prefix': 'your',
                                       'description': 'Careful with those things. They look pointy!', 'location': 12,
                                       'on_person': True, 'moveable': True, 'is_weapon': True, 'base_damage': 10, 'damage': 30,
                                       'hit_bonus': 20, 'no_drop': True})
                        special_done[1]['done'] = True
...
def load_game(choice):
    ...
    if choice == '1':
        ...
        world.insert(12, {'visible': False, 'name': 'Surface', 'prefix': 'floating on the', 'name2': '',
                          'desc': 'The cool water ripples past toward the shore. Through the grainy water, you see a small clearing in the water below. A basic ladder leads out of the lake up to the pier.',
                          'exits': {'u': 11, 'd': 13}})
        ...
        # initialize special vars
        special_done.append({'id': 0, 'done': False}) # transformation message show only when entering surface from pier
        special_done.append({'id': 1, 'done': False}) # has claws - add (true) when in water, remove (false) when out
