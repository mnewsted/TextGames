...
# text_games
# version 1.2.10.3
# description added creature health, creature wellness, player can attack
            ...
            if mob['room']==current_room and mob['is_dead'] == False:
                for thing in things:
                    if thing['name'].lower() == weapon and thing['is_weapon']==False:
                        return
                    print( 'It damages ' + mob['name'] + ' for ' + str(thing['damage']) + ' damage.' )
                    mob['current_hp'] = mob['current_hp'] - thing['damage']
                    mob['is_hostile'] = True
                    if mob['current_hp'] <= 0:
...
def load_game(choice):
    ...
    if choice == '1':
        ...
        starting_room = 0
        exit_room = 2
        things.append({ 'name': 'hammer', 'prefix': 'a', 'description': 'It\'s a classic ball-peen hammer. Could do some real damage.', 'location': 0, 'on_person': False, 'moveable': True, 'is_weapon': True, 'damage': 35})
        ...
        creatures.append({ 'id': 0, 'name': 'Monster', 'description': 'The monster is about 7 feet tall, covered in yellow fur, and shows sharp teeth and claws.', 'room': 0, 'max_hp': 100, 'current_hp': 100, 'is_dead': False, 'is_hostile': False, 'status_neutral': 'It isn\'t interested in you.', 'status_hostile': 'It\'s very mad at you.' })
